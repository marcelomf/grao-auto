  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        text[table] = '{\n  "bundle": "'+tableName+'",\n  "label": "'+self.capitalize(tableName)+
                      '",\n  "description": "All '+self.capitalize(tableName)+'",\n  "refLabel": "",\n  "fields": {\n';
      } else {
      ...
      _.each(fields, function(field, i){
        ...
        text[table] += spaces + spaces + spaces + '"label": '+'"'+self.capitalize(fieldName)+'",\n';
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              ...
              text[table] += spaces + spaces + spaces + '"ref": "'+self.tables[table][field][attr].foreignSources.target_table+'"';
            ...
            if(self.tables[table][field]['primaryKey'] == false)
